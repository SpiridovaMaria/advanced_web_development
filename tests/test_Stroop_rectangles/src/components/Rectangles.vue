...
<template>
  ...
  <h4 v-if='level<=number_of_levels' class='text-center'>Задание: найдите все квадраты, у которых цвет и надпись (название цвета) совпадают.</h4>
  <div v-if='level<=number_of_levels' id='content'>
  ...
  <div v-if='level==number_of_levels+1' class="card">
    <div class="card-header text-center">
      <h3>Результат теста</h3>
    </div>
    <ul class="list-group list-group-flush">
      <li class="list-group-item"><h5><b>Время прохождения:</b> {{ this.$refs.timer.minutes }}:{{ this.$refs.timer.seconds < 10 ? '0' : '' }}{{ this.$refs.timer.seconds }}</h5></li>
      <li class="list-group-item"><h5><b>Количество правильных ответов:</b> {{final_correct_answ}}</h5></li>
    ...
    next(){
      if (this.correct_answ == 0 && this.level<this.number_of_levels){
        ...
        for (let i=0;i<this.level*5;i++){
          ...
          console.log(this.origins[j])
          if (this.origins[j].color == this.origins[j].textColor){
            this.correct_answ+=1
          }
        }
        this.final_correct_answ+=this.correct_answ
      }
